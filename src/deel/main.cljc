(ns deel.main
  "Kotodama WASM entrypoint for the Deel clean-room actor (L4) — Clojure port.
  ...
  facts (`deel.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Contract"  :plural "contracts"  :id-prefix "deel_con"
    :fields [:contractId :type :status :teamId :country :currency :externalId] :required [:type]
    :coerce {}                          :refs {:contractId "Contract"}}
   {:entity "Worker"    :plural "workers"    :id-prefix "deel_wor"
    :fields [:profileId :name :firstName :lastName :email :country] :required [:name :firstName]
    :coerce {}                          :refs {}}
   {:entity "Milestone" :plural "milestones" :id-prefix "deel_mil"
    :fields [:title :amount :status :contractId :createdAt] :required [:title]
    :coerce {:amount :float}            :refs {:contractId "Contract"}}
   {:entity "Timesheet" :plural "timesheets" :id-prefix "deel_tim"
    :fields [:contractId :date :hours :description :status :reporterId] :required [:date]
    :coerce {:hours :float}             :refs {:contractId "Contract"}}
   {:entity "Payment"   :plural "payments"   :id-prefix "deel_pay"
    :fields [:paymentId :dateFrom :dateTo :status :currency :entityType] :required [:dateFrom :dateTo]
    :coerce {}                          :refs {:paymentId "Payment"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"deel.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "deel-compat" :tier tier :entities entities} 200])
